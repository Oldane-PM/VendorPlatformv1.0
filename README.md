...
- **401 on `/api/auth` in production:** Set **`BETTER_AUTH_URL`** and **`NEXT_PUBLIC_APP_URL`** to your real **`https://…up.railway.app`** origin (no `/api/auth` path). If those are missing, Better Auth can fall back to localhost and reject the browser’s `Origin` header. The app also reads **`RAILWAY_PUBLIC_DOMAIN`** when present. **Google Cloud Console:** OAuth redirect URIs must include `https://YOUR_DOMAIN/api/auth/callback/google` (that path is correct here; only `BETTER_AUTH_URL` stays origin-only).
