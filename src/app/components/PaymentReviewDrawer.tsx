...
import { PaymentProcessingDetailDto } from '@/lib/domain/finance/paymentProcessingApiRepo';
...
interface PaymentReviewDrawerProps {
  ...
  paymentDetail?: PaymentProcessingDetailDto | null;
...
export function PaymentReviewDrawer({
  ...
  // Sync state if payment is completed — must be ABOVE the early return
  const isCompleted = invoice?.status === 'Completed';
  useEffect(() => {
    if (isCompleted && paymentDetail) {
      if (paymentDetail.bank_account_id)
        setSelectedBankAccountId(paymentDetail.bank_account_id);
      if (paymentDetail.payment_method)
        setPaymentMethod(paymentDetail.payment_method);
      if (paymentDetail.payment_date)
        setPaymentDate(paymentDetail.payment_date);
      if (paymentDetail.reference_number)
        setReferenceNumber(paymentDetail.reference_number);
      if (paymentDetail.notes) setNotes(paymentDetail.notes);

      setExchangeRateVariance(
        String(paymentDetail.exchange_rate_variance ?? 0)
      );
      setBankTransferFee(String(paymentDetail.bank_transfer_fee ?? 0));
      setGctOnTransaction(String(paymentDetail.gct_on_transaction ?? 0));
      setOtherFees(String(paymentDetail.other_fees ?? 0));
    } else {
      // Reset
      ...
      setReferenceNumber('');
      setNotes('');
      setExchangeRateVariance('0');
      setBankTransferFee('0');
      setGctOnTransaction('0');
      setOtherFees('0');
    }
  ...
  // Calculate totals
  const mappedExchangeVariance = parseFloat(exchangeRateVariance || '0');
  const mappedBankTransferFee = parseFloat(bankTransferFee || '0');
  const mappedGct = parseFloat(gctOnTransaction || '0');
  const mappedOther = parseFloat(otherFees || '0');

  const totalFees =
    isCompleted && paymentDetail
      ? (paymentDetail.exchange_rate_variance ?? 0) +
        (paymentDetail.bank_transfer_fee ?? 0) +
        (paymentDetail.gct_on_transaction ?? 0) +
        (paymentDetail.other_fees ?? 0)
      : mappedExchangeVariance +
        mappedBankTransferFee +
        mappedGct +
        mappedOther;
                        ...
                        {hasInsufficientBalance && (
                          <div className="mt-2 p-3 bg-red-50 border border-red-200 rounded-lg flex items-start gap-2">
                            ...
                            <p className="text-xs text-red-700">
                              <strong>Insufficient Balance:</strong> The
                              selected account does not have enough funds to
                        ...
                        <div>
                          ...
                          <div className="flex items-center gap-2">
                            <input
                              ...
                              onChange={(e) =>
                                setExchangeRateVariance(e.target.value)
                              }
                              placeholder="0.00"
                        ...
                        <div>
                          ...
                          <div className="flex items-center gap-2">
                            <input
                              ...
                              onChange={(e) =>
                                setBankTransferFee(e.target.value)
                              }
                              placeholder="0.00"
                              className="flex-1 px-4 py-2.5 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-primary focus:border-transparent"
                            />
                        ...
                        <div>
                          ...
                          <div className="flex items-center gap-2">
                            <input
                              ...
                              onChange={(e) =>
                                setGctOnTransaction(e.target.value)
                              }
                              placeholder="0.00"
                              className="flex-1 px-4 py-2.5 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-primary focus:border-transparent"
                            />
                      ...
                      <div>
                        ...
                        <p className="text-sm text-green-900">
                          {paymentDetail.payment_date
                            ? formatDate(paymentDetail.payment_date)
                            : '-'}
                        </p>
                      </div>
                    </div>
                    ...
                    {isCompleted && paymentDetail && (
                      <div className="flex items-start gap-3">
                        ...
                        <div>
                          ...
                          <p className="text-xs text-gray-500">
                            {paymentDetail.payment_date
                              ? `Processed on ${formatDate(paymentDetail.payment_date)}`
                              : ''}
                          </p>
                        </div>
                      </div>
